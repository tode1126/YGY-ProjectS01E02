						...
						<c:forEach items="${list }" var="dto" varStatus="i">
							<c:if test="${dto.qna_ref eq '0' }">
								<tr>
									...
									<td><a
										href="qna_boardListContent.do?qna_pk=${dto.qna_pk }&pageNum=${currentPage}">${dto.qna_subject }</a></td>
									<td align="center">${dto.qna_writer }</td>
									<fmt:formatDate var="date" value="${dto.qna_writeday }"
										pattern="yyyy년 MM월 dd일 HH시 mm분" />
									<td align="center">${date }</td>
									<td align="center"><input type="button" value="답글"
										class="blue button"
										onclick="location.href='${root}/admin/qna_boardManagement/qna_boardListReplyInsert.do?ori_qna_pk=${dto.qna_pk }&pageNum=${currentPage }'"></td>
								</tr>
							</c:if>
							<c:forEach items="${list }" var="dto2" varStatus="z">
								<c:if test="${dto2.qna_ref eq dto.qna_pk }">
									<tr>
										<td align="right">답글</td>
										<td><a
											href="qna_boardListContent.do?qna_pk=${dto2.qna_pk }&pageNum=${currentPage}">&nbsp;&nbsp;&nbsp;${dto2.qna_subject }</a></td>
			...
			<div class="pageNum">
				<c:if test="${not empty list }">
					<c:if test="${startPage > 1 }">
						<div>
							<a href="qna_boardList.do?pageNum=${startPage-1 }">◀</a>
						</div>
					</c:if>
					...
					<c:forEach var="pp" begin="${startPage }" end="${endPage }">
						<c:if test="${pp eq currentPage }">
							<div>
								<a href="qna_boardList.do?pageNum=${pp }"
									style="color: red; font-size: bold;">${pp }</a>
							</div>
						</c:if>
						<c:if test="${pp ne currentPage }">
							<div>
								<a href="qna_boardList.do?pageNum=${pp }" style="color: black;">${pp }</a>
							</div>
						</c:if>
					</c:forEach>
					...
					<c:if test="${endPage<totalPage }">
						<div>
							<a href="qna_boardList.do?pageNum=${endPage+1 }">▶</a>
						</div>
					</c:if>
				</c:if>
			</div>
