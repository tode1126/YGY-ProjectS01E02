...
<link rel="stylesheet" href="${root }/css/admin/reboardListStyle.css">
						...
						<c:if test="${not empty list }">
							<c:forEach items="${list }" var="dto" varStatus="i">
								<tr>
									...
									<td><c:forEach var="sp" begin="1" end="${dto.relevel }">
									&nbsp;&nbsp;
									</c:forEach> <c:if test="${dto.relevel>0 }">
											<img width="15"
												src="${root }/image/reboard/iconmonstr-arrow-54-64.png">
										</c:if> <a
										href="reboardListSelectContent.do?reboard_pk=${dto.reboard_pk}&pageNum=${currentPage}">
											${dto.reboard_subject}</a> <c:if test="${dto.answerCount ne 0 }">
									...
									<td align="center"><button type="button"
											class="red button"
											onclick="location.href='reboardListDelete.do?reboard_pk=${dto.reboard_pk}&pageNum=${param.pageNum }'">삭제</button></td>
								</tr>
							</c:forEach>
						</c:if>
					</table>
